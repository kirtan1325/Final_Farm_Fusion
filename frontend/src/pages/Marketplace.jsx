...
function ProductCard({ crop, user }) {
  ...
  return (
    <div className="ff-product-card ff-fade-in">
      <div className={`relative h-44 bg-gradient-to-br ${BG_GRADIENTS[bgIdx]} flex items-center justify-center`}>
        ...
        {crop.badge && (
          <span className={`absolute top-3 left-3 ff-badge ${BADGE_STYLE[crop.badge] || "ff-badge-gray"}`}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {pagedCrops.map((crop, i) => (
                <div key={crop._id} className={`ff-stagger-${Math.min(i % 4 + 1, 4)}`}>
                  <ProductCard crop={crop} user={user} />
                </div>
              ))}
            </div>
          ) : (
