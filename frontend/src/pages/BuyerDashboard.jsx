...
function ProductCard({ crop, staggerClass = "" }) {
  ...
  return (
    <div className={`ff-product-card ff-fade-in ${staggerClass}`}>
      {/* Image / gradient area */}
      <div className={`relative h-44 bg-gradient-to-br ${BG_GRADIENTS[bgIdx]} flex items-center justify-center overflow-hidden`}>
        {/* subtle dark overlay for depth */}
        <div className="absolute inset-0 bg-black/10" />
        <span className="text-7xl select-none relative z-10 drop-shadow-lg">{crop.emoji || "🌾"}</span>
        {crop.badge && (
          <span className={`absolute top-3 left-3 z-10 text-[11px] font-bold px-2.5 py-1 rounded-full backdrop-blur-sm ${BADGE_STYLES[crop.badge] || "bg-white/90 text-gray-800"}`}>
